...
class HikvisionIndustrialCamera {
 ...
 public:
  ...
  void checkError(int _result, const std::string& _operation);
};
...
unsigned int hik_camera::getExposureTime() {
  ...
  checkError(MV_CC_GetFloatValue(handle_, "ExposureTime", &exposure_time),
             "Get exposure time");
  return static_cast<int>(exposure_time.fCurValue);
}
...
unsigned int hik_camera::getGain() {
  ...
  checkError(MV_CC_GetFloatValue(handle_, "Gain", &gain), "Get exposure time");
  return static_cast<int>(gain.fCurValue);
}
...
unsigned int hik_camera::getGamma() {
  ...
  return static_cast<int>(gamma.fCurValue);
}
...
void hik_camera::checkError(int _result, const std::string& _operation) {
  if (_result != MV_OK) {
